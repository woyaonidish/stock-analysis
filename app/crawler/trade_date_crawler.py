...
class TradeDateCrawler(BaseCrawler):
    """交易日历爬虫"""
    ...
    def get_trade_date_hist(self) -> pd.DataFrame:
        """
        获取历史交易日历
        ...
        try:
            ...
            dict_list = js_code.call(
                "d", text_data.split("=")[1].split(";")[0].replace('"', "")
            )
            ...
            temp_list.append(datetime.date(1992, 5, 4))  # 补充缺失的交易日
            temp_list.sort()
            ...
            # 如果py_mini_racer不可用，返回空DataFrame
            return pd.DataFrame(columns=["trade_date"])
    
    def get_trade_dates(self) -> List[datetime.date]:
        """
        获取交易日列表
        ...
        df = self.get_trade_date_hist()
        if df.empty:
            return []
        
        self._trade_dates = df["trade_date"].tolist()
        return self._trade_dates
    ...
    def get_next_trade_date(self, date: datetime.date) -> Optional[datetime.date]:
        """
        获取下一个交易日
    ...
    def get_prev_trade_date(self, date: datetime.date) -> Optional[datetime.date]:
        """
        获取上一个交易日
    ...
    def crawl(self, **kwargs) -> pd.DataFrame:
        """实现爬取方法"""
        return self.get_trade_date_hist()
