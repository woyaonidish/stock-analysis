...
class StockHistCrawler(BaseCrawler):
    ...
    async def async_get_stock_hist(
        self, 
        symbol: str, 
        period: str = "daily",
        start_date: str = "19700101", 
    ...
    async def async_get_stock_hist_min(
        self,
        symbol: str,
        start_date: str = "1979-09-01 09:32:00",
        end_date: str = "2222-01-01 09:32:00",
        ...
        if period == "1":
            ...
            temp_df = pd.DataFrame([item.split(",") for item in data_json["data"]["klines"]])
            temp_df.columns = ["time", "open", "close", "high", "low",
                              "volume", "amount", "amplitude", "change_pct", "change_amount", "turnover"]
    ...
    def crawl(self, symbol: str = "000001", **kwargs) -> pd.DataFrame:
        """实现爬取方法"""
        return self.get_stock_hist(symbol, **kwargs)
    
    async def async_crawl(self, symbol: str = "000001", **kwargs) -> pd.DataFrame:
        """异步爬取方法"""
        return await self.async_get_stock_hist(symbol, **kwargs)